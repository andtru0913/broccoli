  ...
  if (user.admin) {
    ...
    popup = (
      <div>
        ...
        <input className={"form-control block px-3 py-1.5 text-base font-normal text-muted  solid  border  border-slate-900 focus:text-muted focus:border-dashed hover:border-dashed"} id={"file"} type="file" name="myImage" onChange={file.uploadToClient} />
        <button className="btn btn-primary" type="submit" onClick={
              async function() {
                try {
                  file.uploadToServer(`news/${document.getElementById("file").value.split(/([\\/])/g).pop()}`).then(e => {console.log(e)});
