...
const NavItem = ({ text, href, active, theme }) => {
    ...
    return (
        <ActiveLink scroll={false} href={href} activeClassName="w-full text-xs font-medium md:ml-8 text-darkest uppercase opacity-80 transition-all duration-200">
            <a  className={`${theme != "intranet" ? "w-full text-xs md:ml-8 font-medium  text-darkest uppercase opacity-80 transition-all duration-200 hover:text-salmone": "w-full text-xs font-medium md:ml-8 text-white uppercase opacity-80 transition-all duration-200 hover:text-darkest"} `}
