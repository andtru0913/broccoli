        ...
        popup = {
            background : (
                <div id='popup' className={`${popupStyles.popUp} ${popupStyles.hide}`} onClick={function() {
                    ...
                    document.getElementById("createCard").classList.add(popupStyles.hide)
                    document.getElementById("modifyCard").classList.add(popupStyles.hide)
                    document.getElementById("modifyPage").classList.add(popupStyles.hide)
                }}>
                </div>
            ),
            button : (
            ...
            createCard : (
                <div id='createCard' className={`${popupStyles.window}  ${popupStyles.hide}`}>
                    <h1> Skapa kort</h1>
                    <form action="../../api/createCard" method="POST">
                        ...
                        <input className="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700  bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" type="file" id="createFormFile" name="file" onChange={async function() {
                            ...
                            document.getElementById("createBase64").value = await convertBase64(f)
                            submit.disabled = false;
                        }}/>
                        <button id="createCardSubmit" type="submit"> Skapa </button>
                    </form>
                </div>),
            modifyCard: (<div id='modifyCard' className={`${popupStyles.window} ${popupStyles.hide}`}>
                <h1> Ändra kort</h1>
                <form action="../../api/modifyCard" method="POST">
                    ...
                    <input className="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700  bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" type="file" id="modifyFormFile" name="file" onChange={async function() {
                        ...
                        document.getElementById("modifyBase64").value = await convertBase64(f)
                        submit.disabled = false;
                    }}/>
            ...
            modifyPage: (<div id='modifyPage' className={`${popupStyles.window} ${popupStyles.hide}`}>
                <h1> Ändra sida</h1>
